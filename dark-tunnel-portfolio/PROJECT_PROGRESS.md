...
- **Physics/Detection**: Rapier (@react-three/rapier)
- **State Management**: Zustand
- **Animation**: GSAP (free tier)
- **Post-Processing**: @react-three/postprocessing
- **Audio**: Howler.js
- **Utilities**: Drei, gltfjsx
...
### State Management (Zustand)
- `currentTrack`: Active spline curve data
- `progress`: 0–1 position on current track
- `gameState`: "RIDING" | "CHOOSING_PATH" | "INSIDE_CAVE"
- `availablePaths`: Array of upcoming path options
- `selectedProjectData`: Current cave/project info

### Movement System
- **Not physics-based** - Uses `CatmullRomCurve3` from Three.js
- **Input**: Arrow keys (Up/Down) + Mouse (Left/Right click)
- **Camera**: Positioned at `curve.getPointAt(progress)`, facing `curve.getTangentAt(progress)`
- **Progress increment**: Fixed per frame based on input speed

### Visual Transitions
- **Tunnel**: Dark, low ambient light, miner's lantern spotlight
- **Cave Entrance**: Invisible trigger box triggers bloom + lighting intensity increase
- **Projects**: Holographic neon glow on stone pillars (emissive + bloom)
...
### 🟡 PHASE 1: GRAYBOX (Core Logic)
**Goal**: Prove core mechanics work with minimal visuals

#### Tasks:
- [ ] Create Zustand store for game state
- [ ] Implement single spline curve (CatmullRomCurve3)
- [ ] Build handcar movement with Arrow keys
- [ ] Add camera following handcar position
- [ ] Implement curve traversal with proper tangent-based camera rotation
...
**Acceptance Criteria**:
- ✓ Cube/primitive moves smoothly along curve when arrow keys pressed
- ✓ Camera follows cube naturally
- ✓ Progress bar shows 0→100% completion
- ✓ No lag or jank

---

### 🟡 PHASE 1B: BRANCHING LOGIC
**Goal**: Add path selection at curve endpoints

#### Tasks:
- [ ] Create second spline curve
- [ ] Pause movement when progress === 1
- [ ] Display path options (temp: simple HTML popup or debug UI)
- [ ] Handle path selection (keyboard input)
...
**Acceptance Criteria**:
- ✓ Cube stops at curve end
- ✓ 2–3 path options appear
- ✓ Arrow keys select between options
- ✓ Enter key confirms selection
- ✓ Cube smoothly transitions to new curve

---

### ⏳ PHASE 2: ASSETS & ENVIRONMENT
**Goal**: Replace primitives with real 3D models

#### Tasks:
- [ ] Download/source tunnel models (Sketchfab)
- [ ] Download handcar model
- [ ] Download stone pillar models
- [ ] Optimize models with `gltfjsx`
...
### ⏳ PHASE 3: LIGHTING & POST-PROCESSING
**Goal**: Atmospheric effects for tunnel→cave transition

#### Tasks:
- [ ] Implement dark tunnel ambient lighting
- [ ] Add miner's lantern spotlight (handcar-attached)
- [ ] Create cave entrance trigger (Rapier box)
- [ ] Implement bloom post-processing
- [ ] Animate lighting transition (dark→bright)
- [ ] Add GSAP timeline for cinematic entrance
...
### ⏳ PHASE 4: AUDIO & AMBIENCE
**Goal**: Cinematic soundscape

#### Tasks:
- [ ] Source cinematic music (free: Incompetech, YouTube Audio Library)
- [ ] Source ambient sounds (Freesound.org)
- [ ] Integrate Howler.js
- [ ] Sync music with movement (loop/fade)
- [ ] Add wheel rolling sounds
- [ ] Add cave entrance audio cue
...
### ⏳ PHASE 5: HOLOGRAM & PROJECT DISPLAY
**Goal**: Neon holographic project showcases

#### Tasks:
- [ ] Create stone pillar models/placement
- [ ] Design hologram shader (emissive + bloom)
- [ ] Display project info as glowing text
- [ ] Add project images/mockups
- [ ] Create cave layouts for each project
- [ ] Populate with actual portfolio projects
...
- [x] Implemented camera following (`CameraController.tsx`)
...
- Camera system following handcar with lookahead
...
│   │   ├── CameraController.tsx ✓ (camera follow)
...
7. `src/components/CameraController.tsx` - Camera follow system
