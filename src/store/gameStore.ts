...
import { buildJourney, collectAllPathCurves, getForkChoices, getSegmentCurve, syncOverallProgress } from "@/lib/journey";
...
export const useGameStore = create<GameStoreState>((set, get) => ({
  ...
  completeChamber: () => {
    ...
    const currentSegment = graph.segments[state.mainSegmentIndex];
    ...
    set({
      ...
      gameState: "RIDING",
      availablePaths: getForkChoices(currentSegment),
      overallProgress: syncOverallProgress(
        "main",
        state.mainSegmentIndex,
        0.99,
